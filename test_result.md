...
agent_communication:
  - agent: "main"
    message: "NEW FORK (2026-06) - 5-ITEM WORKOUT BUILD IMPROVEMENTS: All 5 items implemented. ITEM 1 (P0 Bug - backend): Added SECONDARY_SYNERGY dict + secondary_injections_this_week counter + MAX_SECONDARY_INJECTIONS=2. Old injection used session_native gate (broke 106/140 combos). New code checks SECONDARY_SYNERGY compatibility, caps at 2/week. ITEM 2 (P0 Bug - frontend workout-questionnaire.tsx): Added 5 missing styles: startDayRow, startDayBtn, startDayBtnActive, startDayText, startDayTextActive. These are needed for the start-day picker buttons to be visible. ITEM 3 (P1 Feature - backend+frontend): generate_weekly_progression() now returns 12 weeks (3x4-week blocks with goal-specific labels+instructions). Frontend: Math.min(12,...) instead of 4, 'Week X of 12' banner, currentWeek===12 completion prompt, weekOptions auto-generates 1-12. ITEM 4 (P2 Polish - frontend): Replaced ActivityIndicator with animated full-screen Modal overlay: pulsing Ionicons flash icon, 'InterFitAI' gold text, cycling status message (4 msgs × 3s interval). ITEM 5 (P2 Polish - frontend+backend): UserProfile model + UserProfileUpdate model now have exercise_preferences: Optional[str]. userStore.ts has exercise_preferences?:string. Questionnaire pre-fills from profile.exercise_preferences, sends to generate payload, PATCHes profile on submit. ACCEPTANCE CRITERIA: (A) Backend: POST /api/workouts/generate with secondary_focus_areas=['core'] + bro split → at least one core pattern (core_stability/core_flexion) in generated workout. (B) Frontend: Start day buttons visible on step 6 (preferred_start_day step). (C) Backend: generate_weekly_progression returns 12 weeks. (D) Backend: weekly_progression has 3 blocks with different label prefixes (Block 2, Block 3). (E) Frontend: workout-detail 'Week X of 12' shows, week picker shows 12 options. (F) Frontend: questionnaire generation loading shows full-screen overlay not spinner. (G) Backend: UserProfile accepts exercise_preferences field. App URL: https://nutrition-debug-1.preview.emergentagent.com" Three changes to server.py only. (1) Added LIMITATION_SYNONYMS dict (80+ entries) above LIMITATION_EXCLUSIONS — maps free-text injury phrasings to 8 canonical keys. e.g. 'sciatica'→lower_back, 'herniated disc'→lower_back, 'rotator cuff'→shoulder, 'runner's knee'→knee, 'ACL'→knee, 'plantar fasciitis'→ankle, 'tennis elbow'→elbow, 'carpal tunnel'→wrist. (2) Added _normalize_limitations() classmethod that applies synonym map + direct key match, replacing raw string matching in both get_exercise_options() and build_blueprint(). (3) Fixed UNSAFE fallback in get_exercise_options(): was `result = filtered if filtered else unique` — now when filtered is empty it falls back to bodyweight/any options from the same pattern (also filtered for safety), and as last resort returns ['Bodyweight Exercise']. ACCEPTANCE CRITERIA: (A) POST /api/workouts/generate with injuries='sciatica' → generated workout must NOT contain 'Conventional Deadlift', 'Good Morning', or 'Back Squat' in any exercise name. (B) POST /api/workouts/generate with injuries='rotator cuff' → must NOT contain 'Barbell Overhead Press' or 'Barbell Bench Press'. (C) POST /api/workouts/generate with injuries='runner knee' → must NOT contain 'Back Squat', 'Lunge', 'Jump Squat'. (D) Post with injuries='none' → generates normally (no regression). (E) Post with injuries='ACL injury' → same as runner's knee exclusions apply. Test user credentials in /app/memory/test_credentials.md. App URL: https://nutrition-debug-1.preview.emergentagent.com." Two changes made to server.py: (1) build_blueprint() return dict now includes 'exercise_preferences': getattr(req, 'exercise_preferences', None) (line ~2826). (2) Prompt construction (line ~3286) now adds advisory guidance when blueprint.get('exercise_preferences') is truthy: 'User exercise preferences: <text>. When an option matches a liked exercise, prefer it. When an option matches a disliked exercise, choose a different option from the list. Never pick an exercise outside the provided Options.' HOW TO TEST: Call POST /api/workouts/generate with exercise_preferences='I hate burpees, love Romanian Deadlifts' — the generated workout should prefer RDLs when they appear in a slot's options list and avoid burpees. ACCEPTANCE CRITERIA: (A) POST /api/workouts/generate with exercise_preferences returns 200 (no errors). (B) The preferences text was accepted and did not crash generation. (C) When preferences mention a disliked exercise (e.g. 'hate burpees'), that exercise should ideally not appear in the generated workout (though this is advisory — if it's the only option it may still appear). TEST WITH: user_id in test_credentials.md. The test should also confirm: without exercise_preferences, generation still returns 200 (no regression). NOTE: This feature requires an actual Claude call — it will take 10-20 seconds." Backend running (pid 517). Three changes made to GET /api/exercises/search in server.py: (1) EXERCISE_NAME_OVERRIDES dict (90+ entries): ExerciseDB raw names → gym-standard display names at response time. e.g. 'barbell full squat' → 'Barbell Back Squat', 'barbell lying triceps extension' → 'Skull Crushers', 'lever leg press' → 'Leg Press Machine'. (2) EXERCISE_TARGET_OVERRIDES dict (60+ entries): compound exercises → all chip IDs they train. Back squat → [legs, glutes], deadlift → [glutes, legs, back], bench press → [chest, triceps], rows → [back, biceps], OHP → [shoulders, triceps]. (3) Updated query logic: $or combines (a) primary target, (b) secondary_muscles array, (c) override exercise names. Added reverse text lookup so 'back squat' finds 'barbell full squat'. Added secondary_muscles index. ACCEPTANCE CRITERIA: (A) GET /api/exercises/search?muscle=legs — 'Barbell Back Squat' MUST appear in results. (B) GET /api/exercises/search?muscle=glutes — 'Barbell Back Squat' MUST appear. (C) GET /api/exercises/search?search=back+squat — at least 1 result, name='Barbell Back Squat'. (D) GET /api/exercises/search?search=skull+crusher — 'Skull Crushers' in results (not 'Barbell Lying Triceps Extension'). (E) GET /api/exercises/search?muscle=triceps — Barbell Bench Press in results. (F) GET /api/exercises/search?muscle=back — Deadlift in results. Use app URL: https://nutrition-debug-1.preview.emergentagent.com." Backend running. Exercise library seeded with 1394 exercises (DB_NAME=test_database). Manual API tests confirm: chest=158 exercises, back=210 exercises, bench press text=32 exercises, pagination works with offset. The Replace Exercise modal useEffect fix (lines 919-943 in workout-detail.tsx) was applied in previous session but NEVER visually verified. TESTS TO RUN: (1) Backend: GET /api/exercises/search?muscle=chest - verify 100+ results, correct structure (id, name, target, gifUrl, etc.). (2) Backend: GET /api/exercises/search?search=deadlift - verify text search works. (3) Backend: GET /api/exercises/search?muscle=back&offset=50 - verify pagination. (4) Frontend: Navigate to an existing workout → open Replace Exercise modal for a Chest exercise → screenshot to verify 'Chest' chip is pre-selected and exercises load. (5) Frontend: Verify 'Recommended Swaps' hints show in modal. (6) Frontend: Verify 'Load More' button appears when total_count > limit. App URL: https://nutrition-debug-1.preview.emergentagent.com. Test user ID: cbd82a69-3a37-48c2-88e8-0fe95081fa4b. NOTE: If DB is empty in this fork, run: POST /api/admin/import-exercises (needs EXERCISEDB_API_KEY). But DB should already be seeded from fork with 1394 exercises."
  - agent: "main"
  ...
  - agent: "main"
    message: |
      COMBINED UPDATE: ALLERGEN SAFETY + MINCE VARIANTS + RATIO DISPLAY (2026-07, commits 61b2c0f + follow-up):
      Part 1 (allergen safety): Module-level ALLERGEN_SYNONYMS map (gluten/nuts/soy/lactose/dairy/eggs/shellfish -> derivatives), expand_banned_terms(), contains_banned_food() with word-stem matching (singular<->plural, gluten-free false-positive protection), strip_banned_and_recompute() (removes banned ingredients + recomputes meal macros via calculate_ingredient_macros). Applied in BOTH post-validators (generation: also recomputes day totals; swap: do_not_use_list synonym-expanded). REFACTOR: INGREDIENT_MACROS + calculate_ingredient_macros moved from nested scope inside generate_meal_plan to module level (dedented, importable). Added top-level "import re".
      Part 2/3: premium/5-star mince keys added to INGREDIENT_MACROS (153,25,0,6); variant rule covers premium/5-star.
      Part 4: ELITE STANDARDS rule 7 = mince lean/fat ratio display "(95/5)"/"(90/10)"/"(80/20)"; dynamic preferred-foods rule renumbered to 8; same ratio rule added to swap IMPORTANT RULES.
